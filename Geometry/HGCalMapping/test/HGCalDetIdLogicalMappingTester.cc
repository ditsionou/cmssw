...
void HGCalDetIdLogicalMappingTester::analyze(const edm::Event&, const edm::EventSetup& iSetup) {
  ...
  siGeom_ = static_cast<const HGCalGeometry *>(caloGeom_->getSubdetectorGeometry(DetId::HGCalHSi, ForwardSubdetector::ForwardEmpty));
  eeGeom_ = static_cast<const HGCalGeometry *>(caloGeom_->getSubdetectorGeometry(DetId::HGCalEE, ForwardSubdetector::ForwardEmpty));

  fillMapFromGeom(DetId::HGCalEE);
  fillMapFromGeom(DetId::HGCalHSi);
  edm::LogInfo("HGCalMapping") << "Found " << foundSiCells_ << " si cells in geom and siCellLocator, " << missingSiCells_ << " cells in geometry but not cell map." << std::endl;

  fillMapFromGeom(DetId::HGCalHSc);
  edm::LogInfo("HGCalMapping") << "Found " << foundSipmCells_ << " sipm cells in geom and sipmCellLocator, " << missingSipmCells_ << " cells in geometry but not cell map." << std::endl;
  edm::LogInfo("HGCalMapping") << "Number of modules in geometry but not found in module map: " << missingModules_ << std::endl;
  ...
  for (const auto& eleIdIt : mappedIds_) {
    ...
    if(newDetId!=eleIdIt.first) {
      if(newDetId.det()==DetId::HGCalHSc){
        ...
        if(makeTextFiles_){
          misMatchSipmCellFile_ << originalId.layer() << " " << originalId.ring() << " " << originalId.iphi() << " " << originalId.type() << " " << sipmDetId.type() << " " <<originalId.sipm() << " " << sipmDetId.sipm() << " " << std::hex << originalId.rawId() << std::dec << " " << std::hex << sipmDetId.rawId() << std::dec << std::endl;
        }
        else{
          edm::LogError("HGCalMapping") << "original raw ID: "  << std::hex << originalId.rawId() << " does not match new raw ID: " << sipmDetId.rawId() << std::dec << " from electronics ID." << std::endl;  
        ...
        if(makeTextFiles_){
          misMatchSiCellFile_ << originalId.layer() << " " << originalId.waferU() << " " << originalId.waferV() << " " << std::hex << originalId.rawId() << std::dec << " " << std::hex << newDetId.rawId() << std::dec << std::endl;
        }
        else{
          edm::LogError("HGCalMapping") << "original raw ID: "  << std::hex << originalId.rawId() << " does not match raw ID: " << newDetId.rawId() << std::dec << " from electronics id." << std::endl;  
...
{ 
  const HGCalGeometry *geom = static_cast<const HGCalGeometry *>(caloGeom_->getSubdetectorGeometry(detType, ForwardSubdetector::ForwardEmpty));
...
HGCalElectronicsId HGCalDetIdLogicalMappingTester::detIdToEleId(DetId detId){
  ...
  if(detId.det()==DetId::HGCalHSi||detId.det()==DetId::HGCalEE){
    ...
    try{
      ...
      missingModules_++;
      if(makeTextFiles_){
        if(siDetId.zside()<0){missModFile_ << layer << " " << modU << " " << modV << " " << isSiPM << std::endl;}
      }
      edm::Exception e(edm::errors::NotFound,"HGCalDetIdLogicalMappingTester::detIdToEleId: Failed to find module in locator");
      throw e;
    }
    ...
    try{ 
      ...
      missingSiCells_++;
      if(makeTextFiles_){
        missSiCellFile_ << siDetId.layer() << " " << siDetId.cellU() << " " << siDetId.cellV() << " " << wafInfo.part << " " << modInfo.isHD << std::endl;
      }
      edm::Exception e(edm::errors::NotFound,"HGCalDetIdLogicalMappingTester::detIdToEleId: Failed to find si cell in map");
      throw e;
    }
    foundSiCells_++; 
    ...
    try{
      ...
      missingModules_++;
      if(makeTextFiles_){
        if(sipmDetId.zside()<0){missModFile_ << layer << " " << modiring << " " << modiphi << " " << isSiPM << std::endl;}
      }
      edm::Exception e(edm::errors::NotFound,"HGCalDetIdLogicalMappingTester::detIdToEleId: Failed to find module in locator");
      throw e;
    }
    ...
    try{ 
      ...
      edm::Exception e(edm::errors::NotFound,"HGCalDetIdLogicalMappingTester::detIdToEleId: Failed to find sipm cell in locator");
      throw e;
    }
    foundSipmCells_++;
...
DetId HGCalDetIdLogicalMappingTester::eIdToDetId(const HGCalElectronicsId eleId){
  ...
  if(modInfo.isSiPM){
    ...
    const HGCScintillatorDetId sipmDetId = sipmCellLocator_.getDetId(eleId, eleId.halfrocChannel(), intzside, modInfo.plane, modInfo.u, modInfo.v);
    return sipmDetId;
  }
  else {
    ...
    if(modInfo.plane>26){detType=DetId::HGCalHSi;}
    ...
    int intzside = (modInfo.zside) ? -1 : 1;
    const HGCSiliconDetId siDetId(detType, intzside, wafInfo.type, modInfo.plane, modInfo.u, modInfo.v, cellInfo.iu, cellInfo.iv);
    return siDetId;
  }
}
