...
public:
  ...
  const std::vector<HGCROCChannelDataFrame<D> >& getChannelData() const { return channelData_; }
  /// \return vector of 32-bit index, the length is the same as getChannelData(), link from channel data to the first common mode on ROC (+0,+1,+2,+3 for all four common modes)
  const std::vector<uint32_t>& getCommonModeIndex() const { return commonModeIndex_; }
  ...
  const std::vector<uint16_t>& getCommonModeData() const { return commonModeData_; }
  const std::vector<uint32_t>& getBadECOND() const { return badECOND_; }
