...
void HGCalUnpacker<D>::parseSLink(
  ...
  for (uint32_t i = 0; i < inputArray.size();) {  // loop through the S-Link
    //----- parse the S-Link header
    if (((inputArray[i] >> kSLinkBOEShift) & kSLinkBOEMask) != config_.sLinkBOE)  // sanity check
      throw cms::Exception("CorruptData") << "Expected a S-Link header, got 0x" << std::hex << inputArray[i] << ".";

    i += 4;  // length of the S-Link header
    ...
    for (uint8_t captureBlock = 0; captureBlock < config_.sLinkCaptureBlockMax;
      ...
      if (((inputArray[i] >> kCaptureBlockReservedShift) & kCaptureBlockReservedMask) !=
          config_.captureBlockReserved)  // sanity check
        throw cms::Exception("CorruptData")
            << "Expected a capture block header, got 0x" << std::hex << inputArray[i] << ".";

      uint64_t captureBlockHeader = ((uint64_t)inputArray[i] << 32) | ((uint64_t)inputArray[i + 1]);
      ...
      for (uint8_t econd = 0; econd < config_.captureBlockECONDMax; econd++) {  // loop through all ECON-Ds
        ...
        if (((inputArray[i] >> kHeaderShift) & kHeaderMask) != config_.econdHeaderMarker)  // sanity check
          throw cms::Exception("CorruptData") << "Expected a ECON-D header, got 0x" << std::hex << inputArray[i] << ".";

        const uint32_t econdHeader = inputArray[i];
        ...
        if ((((captureBlockHeader >> (3 * econd)) & kCaptureBlockECONDStatusMask) != 0b000) ||
          ...
          badECOND_.emplace_back(i - 2);
          i += payloadLength;  // skip the current ECON-D (using the payload length parsed above)

          if (i % 2 != 0)  //TODO: check this
            i += 1;
          continue;  // go to the next ECON-D
        }
        const uint32_t econdBodyStart = i;  // for the ECON-D length check
        //----- parse the ECON-D body
        if (((econdHeader >> kPassThroughShfit) & kPassThroughMask) == 0) {
          // standard ECON-D
          LogDebug("HGCalUnpack") << "Standard ECON-D";
          auto enabledERX = enabledERXMapping(sLink, captureBlock, econd);
          for (uint8_t erx = 0; erx < config_.econdERXMax; erx++) {
            ...
            LogDebug("HGCalUnpack") << "ECON-D:eRx=" << (int)econd << ":" << (int)erx
                                    << ", first word of the eRx header=0x" << std::hex << inputArray[i] << "\n"
                                    << "Extract common mode 0=0x" << std::hex
                                    << ((inputArray[i] >> kCommonmode0Shift) & kCommonmode0Mask) << std::dec
                                    << ", saved at " << commonModeDataSize_ << "\n"
                                    << "Extract common mode 1=0x" << std::hex
                                    << ((inputArray[i] >> kCommonmode1Shift) & kCommonmode1Mask) << std::dec
            ...
            // empty check
            if (((inputArray[i] >> kFormatShift) & kFormatMask) == 1) {  // empty
              LogDebug("HGCalUnpack") << "eRx empty";
              i += 1;    // length of an empty eRx header
              continue;  // go to the next eRx
            }
            // regular
            uint64_t erxHeader = ((uint64_t)inputArray[i] << 32) | ((uint64_t)inputArray[i + 1]);
            i += 2;  // length of a standard eRx header
            ...
            for (uint8_t channel = 0; channel < config_.erxChannelMax; channel++) {  // loop through all channels in eRx
              ...
              const uint32_t tempIndex = bitCounter / 32 + i;
              ...
              LogDebug("HGCalUnpack") << "full word readout=0x" << std::hex << temp << std::dec << ", code=0x"
              ...
              LogDebug("HGCalUnpack") << "extracted channel data=0x" << std::hex
          ...
          for (uint8_t erx = 0; erx < config_.econdERXMax; erx++) {
            ...
            i += 2;  // length of the standard eRx header
            for (uint8_t channel = 0; channel < config_.erxChannelMax; channel++) {  // loop through all channels in eRx
              const HGCalElectronicsId id(sLink, captureBlock, econd, erx, channel);
              commonModeIndex_[channelDataSize_] = commonModeDataSize_ / 4 * 4;
        ...
        i += 1;  // length of the ECON-D trailer
        // check consistency between length unpacked and payload length
        if (i - econdBodyStart != payloadLength)
          throw cms::Exception("CorruptData") << "Mismatch between unpacked and expected payload length\n"
                                              << "  unpacked payload length=" << i - econdBodyStart << "\n"
...
void HGCalUnpacker<D>::parseCaptureBlock(
  ...
  for (uint32_t i = 0; i < inputArray.size();) {  // loop through all capture blocks
    ...
    for (uint8_t econd = 0; econd < config_.captureBlockECONDMax; econd++) {  // loop through all ECON-Ds
      ...
      if (((econdHeader >> kPassThroughShfit) & kPassThroughMask) == 0) {
        ...
        for (uint8_t erx = 0; erx < config_.econdERXMax; erx++) {  // loop through all eRxs
          ...
          // common mode
          LogDebug("HGCalUnpack") << "ECON-D:eRx=" << (int)econd << ":" << (int)erx
                                  << ", first word of the eRx header=0x" << std::hex << inputArray[i] << std::dec
                                  << "\n"
                                  << "Extract common mode 0=0x" << std::hex
                                  << ((inputArray[i] >> kCommonmode0Shift) & kCommonmode0Mask) << std::dec
                                  << ", saved at " << commonModeDataSize_ << "\n"
          ...
          for (uint8_t channel = 0; channel < config_.erxChannelMax; channel++) {  // loop through channels in eRx
            ...
            const uint32_t temp =
                (tempBit == 0) ? inputArray[tempIndex]
                               : (inputArray[tempIndex] << tempBit) | (inputArray[tempIndex + 1] >> (32 - tempBit));
            const uint8_t code = temp >> 28;
            LogDebug("HGCalUnpack") << "full word readout=0x" << std::hex << temp << std::dec << ", code=0x" << std::hex
                                    << (int)code << std::dec;
            // use if and else here
            channelData_[channelDataSize_] = HGCROCChannelDataFrame<D>(
                logicalMapping(id),
            ...
            if (code == 0b0010)
              channelData_[channelDataSize_].fillFlag1(1);
            LogDebug("HGCalUnpack") << "extracted channel data=0x" << std::hex << channelData_[channelDataSize_].raw();
            channelDataSize_++;
          }
          // pad to whole word
          i += bitCounter / 32;
        ...
        LogDebug("HGCalUnpack") << "Pass through ECON-D";
        auto enabledERX = enabledERXMapping(sLink, captureBlock, econd);
      ...
      i += 1;  // length of an ECON-D trailer

      if (i - econdBodyStart != payloadLength)  // check consistency between length unpacked and payload length
        throw cms::Exception("CorruptData") << "Mismatch between unpacked and expected payload length\n"
                                            << "  unpacked payload length=" << i - econdBodyStart << "\n"
                                            << "  expected payload length=" << payloadLength;
      // pad to 2 words
...
void HGCalUnpacker<D>::parseECOND(
  ...
  for (uint32_t i = 0; i < inputArray.size();) {  // loop through all ECON-Ds
    //----- parse the ECON-D header
    // (the second word of ECON-D header contains no information for unpacking, use only the first one)
    if (((inputArray[i] >> kHeaderShift) & kHeaderMask) != config_.econdHeaderMarker)  // sanity check
      throw cms::Exception("CorruptData") << "Expected a ECON-D header, got 0x" << std::hex << inputArray[i] << ".";
    ...
    if (((econdHeader >> kPassThroughShfit) & kPassThroughMask) == 0) {
      ...
      for (uint8_t erx = 0; erx < config_.econdERXMax; erx++) {  // loop through all eRxs
        ...
        if (((inputArray[i] >> kFormatShift) & kFormatMask) == 1) {  // empty eRx
          LogDebug("HGCalUnpack") << "eRx empty";
          i += 1;  // length of an empty eRx header
